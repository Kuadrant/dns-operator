...
func CopyLabel(label string, from, to *DNSTreeNode) {
	...
	if toNode != nil {
		...
		for _, fd := range from.Endpoints {
			for _, td := range toNode.Endpoints {
...
func AddLabelToTree(node *DNSTreeNode, label, value string) {
	if len(node.Endpoints) == 0 {
		node.Endpoints = append(node.Endpoints, &endpoint.Endpoint{Labels: map[string]string{label: value}})
	} else {
		for _, ep := range node.Endpoints {
			if ep.Labels == nil {
				ep.Labels = endpoint.NewLabels()
