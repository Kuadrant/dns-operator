...
func (r *MultClusterDNSRecordReconciler) Reconcile(ctx context.Context, req mcreconcile.Request) (ctrl.Result, error) {
	...
	_, err := r.mgr.GetCluster(ctx, req.ClusterName)
	if err != nil {
		return reconcile.Result{}, err
	}

	//ToDo Continue processing the record from the remote cluster
...
func (r *MultClusterDNSRecordReconciler) SetupWithManager(mgr mcmanager.Manager) error {
	r.mgr = mgr
