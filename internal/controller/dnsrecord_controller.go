...
func (r *DNSRecordReconciler) updateStatus(ctx context.Context, previous, current DNSRecordAccessor, hadChanges bool, specErr error) (reconcile.Result, error) {
	...
	if specErr != nil {
		...
		return ctrl.Result{RequeueAfter: defaultValidationRequeue}, updateError
	}

	readyCond := meta.FindStatusCondition(current.GetStatus().Conditions, string(v1alpha1.ConditionTypeReady))

	// Determine requeue duration based on changes and state
	...
	if hadChanges {
		...
		// after changes, quick validation requeue
		requeueAfter = defaultValidationRequeue
	} else {
		logger.Info("All records are already up to date")

		if readyCond == nil || generationChanged(current.GetDNSRecord()) {
	...
	return r.updateStatusAndRequeue(ctx, r.Client, previous, current, requeueAfter)
}
