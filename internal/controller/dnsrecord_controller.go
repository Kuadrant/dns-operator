...
func (r *DNSRecordReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// Update the logger with appropriate record/zone metadata
	...
	if dnsRecord.DeletionTimestamp != nil && !dnsRecord.DeletionTimestamp.IsZero() {
		...
		if dnsRecord.Status.ZoneID != "" && dnsRecord.Status.ZoneDomainName != "" {
			// Create a dns provider with config calculated for the current dns record status (Last successful)
			dnsProvider, err := r.ProviderFactory.ProviderFor(ctx, dnsRecord, provider.Config{
			...
			logger.Info(fmt.Sprintf("unable to perform cleanup of dns provider for zone status id: '%s', dominaName: '%s'",
				dnsRecord.Status.ZoneID, dnsRecord.Status.ZoneDomainName))
		}
	...
	// Ensure ZoneID and ZoneDomainName are set in the status
	// Handles both the case where we have never assigned a zone to a DNSRecord (first time reconcile) and when the
	// `rootHost` has changed and potentially needs assigned to a different zone.
	if dnsRecord.Status.ZoneID == "" || dnsRecord.Spec.RootHost != dnsRecord.Status.RootHost {
		logger.Info(fmt.Sprintf("root host changing from %s to %s, updating zone config",
			dnsRecord.Status.RootHost, dnsRecord.Spec.RootHost))
		...
		z, err := dnsProvider.DNSZoneForHost(ctx, dnsRecord.Spec.RootHost)
		if err != nil {
			setDNSRecordCondition(dnsRecord, string(v1alpha1.ConditionTypeReady), metav1.ConditionFalse,
	...
	// Verify the zone status is set
	// This shouldn't happen, but continuing from here with invalid filters is likely to cause unexpected changes being
	// made in dns providers which we should always try and avoid.
	if dnsRecord.Status.ZoneID == "" || dnsRecord.Status.ZoneDomainName == "" {
		setDNSRecordCondition(dnsRecord, string(v1alpha1.ConditionTypeReady), metav1.ConditionFalse,
			"InvalidZoneConfig", "invalid zone config")
		return r.updateStatus(ctx, previous, dnsRecord, false, err)
	}

	//Create zone filters, should always include the current zone id and domain name set in the current record status
	...
	// If the zone id and zone domain name have changed, append the previous record values into the provider zone filters.
	// This is required to ensure the old entries are cleaned up from the provider as well as the new ones being created.
	if previous.Status.ZoneID != "" && previous.Status.ZoneID != dnsRecord.Status.ZoneID {
		pConfig.ZoneIDFilter.ZoneIDs = append(pConfig.ZoneIDFilter.ZoneIDs, previous.Status.ZoneID)
	}
	if previous.Status.ZoneDomainName != "" && previous.Status.ZoneDomainName != dnsRecord.Status.ZoneDomainName {
		pConfig.DomainFilter.Filters = append(pConfig.DomainFilter.Filters, previous.Status.ZoneDomainName)
	}

	// Create a dns provider with config calculated from the current and previous dns record statuses
	dnsProvider, err := r.ProviderFactory.ProviderFor(ctx, dnsRecord, pConfig)
	if err != nil {
		setDNSRecordCondition(dnsRecord, string(v1alpha1.ConditionTypeReady), metav1.ConditionFalse,
			"DNSProviderError", fmt.Sprintf("The dns provider could not be loaded: %v", err))
		return r.updateStatus(ctx, previous, dnsRecord, false, err)
...
func (r *DNSRecordReconciler) updateStatus(ctx context.Context, previous, current *v1alpha1.DNSRecord, hadChanges bool, specErr error) (reconcile.Result, error) {
	...
	current.Status.RootHost = current.Spec.RootHost
...
func (r *DNSRecordReconciler) applyChanges(ctx context.Context, dnsRecord *v1alpha1.DNSRecord, dnsProvider provider.Provider, isDelete bool) (bool, error) {
	...
	domainFilter := externaldnsendpoint.NewDomainFilter([]string{rootDomainName})

	//If we are changing from one root host to another we need to include the old root host to ensure proper cleanup
	if dnsRecord.Status.RootHost != "" && dnsRecord.Spec.RootHost != dnsRecord.Status.RootHost {
		domainFilter.Filters = append(domainFilter.Filters, dnsRecord.Status.RootHost)
	}
	...
	plan := externaldnsplan.NewPlan(ctx, zoneEndpoints, statusEndpoints, specEndpoints, []externaldnsplan.Policy{policy},
		externaldnsendpoint.MatchAllDomainFilters{&domainFilter}, managedDNSRecordTypes, excludeDNSRecordTypes,
