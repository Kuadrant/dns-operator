...
func performProbe(ctx context.Context, k8sClient client.Client, probe *v1alpha1.DNSHealthCheckProbe) ProbeResult {
	...
	ips := probe.GetIPAddresses()

	// Default port to 80
	port := 80
	if probe.Spec.Port != nil {
		port = *probe.Spec.Port
	}
	...
	for _, ip := range ips {
		result := performRequest(ctx, string(probe.Spec.Protocol), probe.Spec.Hostname, probe.Spec.Path, ip, port, probe.Spec.AllowInsecureCertificate, headers)
