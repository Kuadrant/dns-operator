...
import (
	...
	externaldnsprovidergoogle "github.com/kuadrant/dns-operator/internal/external-dns/provider/google"
...
type GoogleDNSProvider struct {
	*externaldnsprovidergoogle.GoogleProvider
	googleConfig externaldnsprovidergoogle.GoogleConfig
	logger       logr.Logger
...
func NewProviderFromSecret(ctx context.Context, s *v1.Secret, c provider.Config) (provider.Provider, error) {
	...
	googleProvider, err := externaldnsprovidergoogle.NewGoogleProviderWithService(ctx, googleConfig, dnsClient)
	if err != nil {
