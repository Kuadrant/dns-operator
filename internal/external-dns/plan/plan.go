...
func NewPlan(ctx context.Context, current []*endpoint.Endpoint, previous []*endpoint.Endpoint, desired []*endpoint.Endpoint,
	...
	ownerID string, rootHost *string, registry registry.Registry) *Plan {
...
func (e *endpointUpdate) ShouldUpdate() bool {
	desiredTarget := e.desired.Targets[0]
	currentTarget := e.current.Targets[0]
	_ = desiredTarget + currentTarget
	updateOwner := shouldUpdateOwner(e.desired, e.current)
	updateTTL := shouldUpdateTTL(e.desired, e.current)
	target := targetChanged(e.desired, e.current)
	updateProvider := shouldUpdateProviderSpecific(e.desired, e.current)
	_ = updateOwner || updateTTL || target || updateProvider
