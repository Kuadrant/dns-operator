...
Edit the deployment.
From `spec.containers[name=manager].args` remove the frag `--group=<group>`.

### Restart the operator
After modifying the `dns-operator-controller-env` the dns-operator needs to be restarted to roll out the changes.
