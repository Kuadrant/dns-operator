...
var rootCmd = &cobra.Command{
	...
	Short: "Create a kubeconfig secret for the pod lister controller",
}
...
func init() {
	rootCmd.Flags().StringVarP(&contextName, "context", "c", "", "kubeconfig context to use (required)")
...
func secretGeneration(_ *cobra.Command, _ []string) error {
	log = logf.Log.WithName("secert-generation")

	dirtyStr := os.Getenv("KUBECTL_DNS_DIRTY")
	log.V(1).Info("envVar", "dirtyStr", dirtyStr)
	...
	log.Info("The operator should now be able to discover, and connec to this cluster")
...
func getClusterCA(log logr.Logger) (string, error) {
	...
	clusterCARaw, err := exec.Command(kubectl, args...).Output()
	if err != nil {
		log.V(1).Error(err, "unable to get certificate authority from kubeconfig", "context", contextName)
...
func getClusterServer(log logr.Logger) (string, error) {
	...
	clusterServerRaw, err := exec.Command(kubectl, args...).Output()
	if err != nil {
		log.V(1).Error(err, "unable to get server url from kubeconfig", "context", contextName)
...
func generateServiceAccountToken(log logr.Logger) (string, error) {
	...
	serviceAccountToken, err := exec.Command(kubectl, args...).Output()
	if err != nil {
		log.V(1).Error(err, "unable to generate service account token from kubeconfig", "context", contextName)
		return "", fmt.Errorf("unable to generate service accunt token for %v from kubeconfig", contextName)
	}
...
func defineKubeConfig(log logr.Logger, clusterServer, clusterCA, serviceAccountToken string) Config {
	log.V(1).Info("Create a new kubeconfig using the service account token")
	return Config{
		APIVersion: "v1",
		Kind:       "Config",
...
func verifyKubeConfigConnection(log logr.Logger, kubeConfigPath string) error {
	...
	_, err := cmd.Output()
	if err != nil {
		log.V(0).Info("unable to connect to cluster cluster")
		log.V(1).Info("command", "cmd", strings.Join(cmd.Args, " "), "stderr", cmd.Stderr, "stdout", cmd.Stdout)
...
func generateSecret(log logr.Logger, kubeConfig Config) (*Secret, error) {
	log.V(1).Info("Generate and apply the secret")
	data, err := yaml.Marshal(kubeConfig)
	if err != nil {
		log.Error(err, "marshaling yaml of kubeConfig file")
