...
func removeActiveGroup(_ *cobra.Command, args []string) error {
	...
	for _, zone := range allZones {
		...
		txtRegistry, err := common.GetDefaultTXTRegistry(ctx, providerForZone, "empty")
		if err != nil {
		...
		affectedEndpoints := slice.Filter(endpoints, func(e *externaldnsendpoint.Endpoint) bool {
			return strings.Contains(e.Labels[TXTRecordGroupKey], groupName)
		})
		...
		if len(affectedEndpoints) != 0 && !assumeYes {
