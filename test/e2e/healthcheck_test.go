...
var _ = Describe("Health Check Test", Serial, Labels{"health_checks"}, func() {
	...
	Context("On-cluster healthchecks", func() {
		It("should set healthy status to false when health check fails", func(ctx SpecContext) {
			...
			Expect(probe.Spec.Address).To(Equal(testTargetIP))
			Expect(probe.Spec.Hostname).To(Equal(testHostname))
			Expect(probe.Spec.Port).To(Equal(testPort))
			Expect(probe.Spec.Protocol).To(Equal(v1alpha1.HttpProtocol))
			Expect(probe.Spec.FailureThreshold).To(Equal(1))
		...
		It("should track consecutive failures with custom threshold and interval", func(ctx SpecContext) {
			...
			Expect(probe.Spec.Port).To(Equal(testPort))
			Expect(probe.Spec.Protocol).To(Equal(v1alpha1.HttpProtocol))
			Expect(probe.Spec.FailureThreshold).To(Equal(5))
			...
			err = k8sClient.Get(ctx, client.ObjectKeyFromObject(probe), probe)
			Expect(err).ToNot(HaveOccurred())
			if probe.Status.ConsecutiveFailures <= 5 {
				// Should still be nil or true since we haven't exceeded the threshold
				if probe.Status.Healthy != nil {
					Expect(*probe.Status.Healthy).To(BeTrue(), "Expected probe to still be healthy before threshold is exceeded")
				}
			}
		...
		It("should set healthy status to true when health check passes", func(ctx SpecContext) {
			testPort := 18080
			testHostname := testID + "." + testZoneDomainName
			var testTargetIP string

			// Check if the operator is deployed in-cluster
			deployment := &appsv1.Deployment{}
			err := k8sClient.Get(ctx, client.ObjectKey{
				Name:      "dns-operator-controller-manager",
				Namespace: "dns-operator-system",
			}, deployment)
			operatorDeployed := err == nil
			...
			if operatorDeployed {
						...
						Containers: []v1.Container{
							{
								...
								Args: []string{
									"netexec",
									"--http-port", fmt.Sprintf("%d", testPort),
								},
				...
				server := &http.Server{
					Addr:    fmt.Sprintf("%s:%d", testTargetIP, testPort),
					Handler: handler,
				}
				...
				resp, err := http.Get(fmt.Sprintf("http://%s:%d/healthz", testTargetIP, testPort))
				Expect(err).ToNot(HaveOccurred())
		...
		It("should delete health check probes when DNSRecord is deleted", func(ctx SpecContext) {
			...
			// Create DNSRecord with health check
			dnsRecord = &v1alpha1.DNSRecord{
				ObjectMeta: metav1.ObjectMeta{
					Name:      testID,
					Namespace: testDNSProviderSecret.Namespace,
