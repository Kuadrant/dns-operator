...
var _ = Describe("Single Record Test", Labels{"single_record"}, func() {
	...
	BeforeEach(func(ctx SpecContext) {
		...
		if testDNSProvider == string(provider.DNSProviderEndpoint) {
			...
			Expect(k8sClient.Create(ctx, zoneRecord, &client.CreateOptions{})).To(BeNil())
		}
	})
	...
	AfterEach(func(ctx SpecContext) {
		...
		if testDNSProvider == string(provider.DNSProviderEndpoint) {
			...
			Eventually(func(g Gomega, ctx context.Context) {
				g.Expect(apierrors.IsNotFound(k8sClient.Get(ctx, client.ObjectKeyFromObject(zoneRecord), &v1alpha1.DNSRecord{}, &client.GetOptions{}))).To(BeTrue())
	...
	It("correctly handles wildcard rootHost values", func(ctx SpecContext) {
		...
		expectedElementMatchers := []types.GomegaMatcher{
			PointTo(MatchFields(IgnoreExtras, Fields{
				...
				"RecordType":    Equal("A"),
				"SetIdentifier": Equal(""),
				"RecordTTL":     Equal(externaldnsendpoint.TTL(builderDefaults.DefaultTTL)),
			})),
			PointTo(MatchFields(IgnoreExtras, Fields{
			...
			if testProvider.Name() == provider.DNSProviderGCP {
				...
				Expect(zoneEndpoints).To(ContainElements(
					...
					PointTo(MatchFields(IgnoreExtras, Fields{
						...
						"SetIdentifier": Equal(""),
						"RecordTTL":     Equal(externaldnsendpoint.TTL(builderDefaults.DefaultLoadBalancedTTL)),
					})),
					PointTo(MatchFields(IgnoreExtras, Fields{
