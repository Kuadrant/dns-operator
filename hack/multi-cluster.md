...
##### Locally("make run")
```shell
make create-kubeconfig-secret NAMESPACE=dns-operator-system NAME=kind-kuadrant-dns-local-2 TARGET_CONTEXT=kind-kuadrant-dns-local-1 REMOTE_CONTEXT=kind-kuadrant-dns-local-2 SERVICE_ACCOUNT=dns-operator-controller-manager
```

#####  Remote("make deploy")
```shell
make create-kubeconfig-secret-kind-internal NAMESPACE=dns-operator-system NAME=kind-kuadrant-dns-local-2 TARGET_CONTEXT=kind-kuadrant-dns-local-1 REMOTE_CONTEXT=kind-kuadrant-dns-local-2 SERVICE_ACCOUNT=dns-operator-controller-manager
```
