...
// WithZoneDomainFilter sets the zone domain filters used to filter zones returned by the provider.
// Zones that do not have a dns name that matches any of the filters will be excluded.
// Defaults to the empty list.
func (pb *ProviderBuilder) WithZoneDomainFilter(domains ...string) *ProviderBuilder {
	if val, ok := pb.strDataItems[v1alpha1.ZoneDomainFilterKey]; ok {
		domains = append(strings.Split(val, ","), domains...)
	}
	return pb.WithDataItem(v1alpha1.ZoneDomainFilterKey, strings.Join(domains, ","))
}

// WithZoneIDFilter sets the zone id filters used to filter zones returned by the provider.
// Zones that do not have an id that matches any of the filters will be excluded.
// Defaults to the empty list.
func (pb *ProviderBuilder) WithZoneIDFilter(ids ...string) *ProviderBuilder {
	if val, ok := pb.strDataItems[v1alpha1.ZoneIDFilterKey]; ok {
		ids = append(strings.Split(val, ","), ids...)
	}
	return pb.WithDataItem(v1alpha1.ZoneIDFilterKey, strings.Join(ids, ","))
}
