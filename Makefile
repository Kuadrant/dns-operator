...
run-with-probes: manifests generate fmt vet ## Run a controller from your host.
	go run -ldflags "-X main.gitSHA=${GIT_SHA} -X main.dirty=${DIRTY}" ./cmd/main.go --zap-devel --provider inmemory,aws,google,azure --e2e-test-only-use-mock-health-transport=${MOCK_HEALTH_CHECKS}
